function [x,FVAL,EXITFLAG,OUTPUT,GRAD,HESSIAN] = fminunc_new(FUN,x,options,varargin)
...
% If options are passed in as an optimoptions object, check if there are
% warnings to display related to the algorithm default transition. (We
% create the warnings here before the options are converted into a
% structure.)
if isa(options, 'optim.options.SolverOptions')
    [transitionMsgID,transitionMsgTxt] = algorithmDefaultTransitionMessages(options);
else
    % No transition warnings to display
    transitionMsgID = []; transitionMsgTxt = '';
end
...
% Throw warning related to algorithm default transition, if there is any.
