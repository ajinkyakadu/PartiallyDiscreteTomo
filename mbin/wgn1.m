...
% --- Seed 
if(~isempty(seed)) 
   if(any([~isreal(seed) (length(seed)>1) (isempty(seed)) any((seed-floor(seed))~=0)])) 
...
switch lower(pMode) 
   case 'linear' 
      noisePower = p; 
   case 'dbw' 
      noisePower = power(10,p/10); % 10^(p/10); 
   case 'dbm' 
...
if(strcmp(cplxMode,'complex')) 
   z = randn(2*row,col); 
   y = (sqrt(imp*noisePower/2))*(z(1:row,:)+1i*z(row+1:end,:)); 
else 
