...
classdef opTomoU < opSpot
    ...
    methods
        ...
        % opTomo - constructor
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function op = opTomoU(type, proj_geom, vol_geom, H1, L1, gpu_index)
            
            if nargin < 6 || isempty(gpu_index), gpu_index = 0; end
            
            proj_size = astra_geom_size(proj_geom);
            vol_size  = astra_geom_size(vol_geom);
            
            % construct operator
            op = op@opSpot('opTomo', prod(proj_size)+prod(vol_size), prod(vol_size));
    ...
    methods( Access = private )
        ...
        function y = opTomo_intrnl2D(op,x,mode)
            ...
            if mode == 1        
                ...
                x1  = reshape(x1, op.proj_size);
                
                % Matlab data copied to ASTRA data
                astra_mex_data2d('store', op.sino_id, x1);
                ...
                % extra part
                y = op.H1*double(y(:)) +(op.L1)'*double(x2);
            end
        end % opTomo_intrnl2D
