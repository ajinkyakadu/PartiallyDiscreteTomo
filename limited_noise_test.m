...
[im,bgIm,imS,bgImS] = createPhantom(0:1/(n-1):1,0:1/(n-1):1,modelOpt); % object of size 256 x 256
x  = im(:);
xS = imS(:);         
...
sinogramS = reshape(pS, W.proj_size);
sinogramN = reshape(pN, W.proj_size); % look at how noise has been added
sinogramSN= reshape(pSN, W.proj_size); 
...
tic;
x_dart      = astra.dart(sinogramSN(:), proj_geom, vol_geom, greyValues, initial_arm_it, ...
...
sinogram1=astra_mex_data2d('get', sinogram_id);
noiseV = reshape(noiseV,size(sinogram1));
astra_mex_data2d('store', sinogram_id, sinogram1+noiseV);

% proj_id = astra_mex_data2d('create','-vol',proj_geom,p);
vol_id = astra_mex_data2d('create','-vol',vol_geom,0);

if mtype<3; tau=0.6; else; tau = 0.8; end;
rho = 1;
